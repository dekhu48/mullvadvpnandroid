...
function fillApiAccessMethodSetting<T extends grpcTypes.NewAccessMethodSetting>(
  ...
  switch (method.type) {
    ...
    case 'socks5-local': {
      const socks5Local = new grpcTypes.AccessMethod.Socks5Local();
    ...
    case 'socks5-remote': {
      const socks5Remote = new grpcTypes.AccessMethod.Socks5Remote();
      socks5Remote.setIp(method.ip);
      socks5Remote.setPort(method.port);
      if (method.authentication !== undefined) {
        socks5Remote.setAuthentication(convertToSocksAuth(method.authentication));
      }
      accessMethod.setSocks5remote(socks5Remote);
      break;
    }
    case 'shadowsocks': {
      const shadowsocks = new grpcTypes.AccessMethod.Shadowsocks();
...
function convertToSocksAuth(authentication: SocksAuth): grpcTypes.AccessMethod.SocksAuth {
  const auth = new grpcTypes.AccessMethod.SocksAuth();
...
function convertFromAccessMethod(method: grpcTypes.AccessMethod): AccessMethod {
  switch (method.getAccessMethodCase()) {
    ...
    case grpcTypes.AccessMethod.AccessMethodCase.SOCKS5REMOTE: {
      ...
      const auth = socks5Remote.getAuthentication();
      return {
        type: 'socks5-remote',
...
function convertFromSocksAuth(auth: grpcTypes.AccessMethod.SocksAuth): SocksAuth {
  return {
