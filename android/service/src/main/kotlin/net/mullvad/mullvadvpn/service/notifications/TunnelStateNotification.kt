...
class TunnelStateNotification(val context: Context) {
    ...
    var tunnelState by observable<TunnelState>(TunnelState.Disconnected) { _, _, newState ->
        ...
        val shouldBeginReconnecting =
            (newState as? TunnelState.Disconnecting)?.actionAfterDisconnect == ActionAfterDisconnect.Reconnect
    ...
    private fun update() {
        if (context.isNotificationPermissionGranted() && visible && (!reconnecting || !showingReconnecting)) {
